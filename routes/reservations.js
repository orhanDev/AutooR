        ...
        try {
            reservations = await pool.query(
                `SELECT id, booking_id, vehicle_name, vehicle_image, pickup_location, 
                        ...
                        check_in_completed, check_in_date, check_out_completed, check_out_date
                 FROM reservations 
                 WHERE user_id = $1 
            ...
            console.log('Check-in columns not found, using fallback query. Error:', columnError.message);
            try {
                reservations = await pool.query(
                    `SELECT id, booking_id, vehicle_name, vehicle_image, pickup_location, 
                            return_location, pickup_date, pickup_time, return_date, return_time,
                ...
                reservations.rows = reservations.rows.map(row => ({
                    ...row,
                    check_in_completed: false,
                    check_in_date: null,
                    check_out_completed: false,
                    check_out_date: null
                }));
            } catch (fallbackError) {
...
router.put('/:bookingId/check-in', async (req, res) => {
    try {
        const { bookingId } = req.params;
        const { driverLicenseNumber, creditCardNumber } = req.body;
        const reservation = await pool.query(
            `UPDATE reservations SET
                status = 'checked-in',
                check_in_date = CURRENT_TIMESTAMP,
                driver_license_number = COALESCE($1, driver_license_number),
                check_in_completed = true,
                updated_at = CURRENT_TIMESTAMP
             WHERE booking_id = $2
             RETURNING id, booking_id, vehicle_name, status, check_in_date`,
            [driverLicenseNumber, bookingId]
        );
...
router.put('/:bookingId/check-out', async (req, res) => {
    try {
        const { bookingId } = req.params;
        const { vehicleCondition, notes } = req.body;
        
        const reservation = await pool.query(
            `UPDATE reservations SET
                status = 'completed',
                check_out_date = CURRENT_TIMESTAMP,
                vehicle_condition = COALESCE($1, vehicle_condition),
                check_out_notes = COALESCE($2, check_out_notes),
                updated_at = CURRENT_TIMESTAMP
             WHERE booking_id = $3
             RETURNING id, booking_id, vehicle_name, status, check_out_date`,
            [vehicleCondition, notes, bookingId]
        );
        ...
        res.json({
            success: true,
            message: 'Check-out erfolgreich abgeschlossen',
            reservation: reservation.rows[0]
        });
    } catch (error) {
...
router.delete('/:bookingId', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: 'Reservierung erfolgreich storniert (nicht gelöscht)',
            reservation: reservation.rows[0]
        });
    } catch (error) {
        console.error('Fehler beim Stornieren der Reservierung:', error);
