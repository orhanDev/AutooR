-- Fiyatları gerçekçi Alman araç kiralama fiyatlarına güncelle (€/gün)
-- Ekonomik araçlar
...
-- Diğer araçların fiyatlarını %60 düşür (minimum €20)
UPDATE cars SET daily_rate = GREATEST(daily_rate * 0.4, 20) WHERE daily_rate > 100;

-- Güncel fiyatları göster
SELECT make, model, year, daily_rate FROM cars ORDER BY daily_rate DESC LIMIT 10;
