...
function formatCityNameForStorage(cityName) {
...
document.addEventListener('DOMContentLoaded', () => {
                ...
                <div class="col-lg-4">
                    <div class="bg-white rounded-4 p-4 shadow-sm border h-100 d-flex flex-column">
                        ...
                        <div class="d-flex gap-3 mt-4">
                            <a href="/reservation" class="btn btn-secondary flex-fill" style="background:#7B7668; border-color:#7B7668; color:#ffffff;">
                                <i class="bi bi-arrow-left me-2"></i>
                                Zurück
                            </a>
                            <button class="btn flex-fill" style="background:#7B7668; border-color:#7B7668; color:#ffffff;" onclick="processPayment()" disabled id="pay-button">
            ...
            <div class="credit-card-overlay" id="credit-card-overlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 9999; display: flex; align-items: center; justify-content: center;">
                <div class="credit-card-modal" style="background: white; padding: 30px; border-radius: 10px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
                    ...
                    <form id="credit-card-form">
                        ...
                        <button type="button" class="btn" onclick="closeCreditCardForm()" style="background:#7B7668; color:#ffffff; border-color:#7B7668;">Abbrechen</button>
                        <button type="submit" class="btn" style="background:#7B7668; color:#ffffff; border-color:#7B7668;">Zahlung bestätigen</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            ...
                            <h5 class="mb-3 fw-bold">Reservierung erfolgreich!</h5>
                            <p class="mb-2 text-muted">
                                Demo-Modus: Zahlung nicht möglich.
                            </p>
                            <p class="mb-0">
                                Ihr Fahrzeug wurde reserviert.
                            </p>
                        </div>
                        <div class="modal-footer justify-content-center gap-2">
                            <button type="button" class="btn" data-bs-dismiss="modal" style="background:#7B7668; color:#ffffff; border-color:#7B7668;">
                                Verstanden
                            </button>
                            <button type="button" class="btn" onclick="goToBuchungen()" style="background:#7B7668; color:#ffffff; border-color:#7B7668;">
    ...
    function saveReservation() {
        try {
            ...
            if (!reservationData || !selectedVehicle || !selectedVehicle.car_id) {
                console.warn('Cannot save reservation: missing data', {
                    hasReservationData: !!reservationData,
                    hasSelectedVehicle: !!selectedVehicle,
                    vehicleId: selectedVehicle?.car_id
                });
                return;
            }
            ...
            if (sessionCurrentUser) {
                try {
                    const currentUser = JSON.parse(sessionCurrentUser);
                    if (currentUser.email) {
                        userEmail = currentUser.email;
                    }
                } catch (e) {
                    console.error('Error parsing sessionStorage currentUser:', e);
                }
            }
            ...
            if (!userEmail) {
                userEmail = localStorage.getItem('userEmail') || 
                           ...
                           (reservationData && reservationData.email);
            }

            if (!userEmail) {
                console.error('Cannot save reservation: No user email found');
                alert('Fehler: Keine Benutzer-E-Mail gefunden. Bitte melden Sie sich an.');
                return;
            }
            
            console.log('Saving reservation with email:', userEmail);

            const pickupDate = reservationData.pickupDate || searchData.pickupDate;
            const dropoffDate = reservationData.dropoffDate || searchData.dropoffDate;
            const pickupTime = reservationData.pickupTime || '08:00';
            const dropoffTime = reservationData.dropoffTime || '18:00';
            ...
            const booking = {
                ...
                vehicle: {
                    ...
                    image_url: selectedVehicle.image_url || selectedVehicle.image,
                    daily_rate: selectedVehicle.daily_rate || selectedVehicle.dailyRate,
                    transmission_type: selectedVehicle.transmission_type || selectedVehicle.transmission || 'Automatik',
                    fuel_type: selectedVehicle.fuel_type || selectedVehicle.fuelType || 'Benzin'
                },
                pickupDate: pickupDateTime || pickupDate,
                dropoffDate: dropoffDateTime || dropoffDate,
                pickupTime: pickupTime,
                dropoffTime: dropoffTime,
                pickupLocation: formatCityNameForStorage(reservationData.pickupLocation || reservationData.pickupLocationName || searchData.pickupLocation || 'Nicht angegeben'),
                dropoffLocation: formatCityNameForStorage(reservationData.dropoffLocation || reservationData.dropoffLocationName || searchData.dropoffLocation || reservationData.pickupLocation || 'Nicht angegeben'),
                days: reservationData.days || searchData.days || 1,
                basePrice: reservationData.basePrice || reservationData.base_price || 0,
                insuranceAmount: reservationData.insuranceAmount || reservationData.insurance_amount || 0,
                extrasAmount: reservationData.extrasAmount || reservationData.extras_amount || 0,
                totalPrice: reservationData.totalPrice || reservationData.total_price || 0,
                paymentMethod: selectedPaymentMethod || 'demo',
                status: 'reserviert',
            ...
            let bookings = [];
            const existingBookingsStr = localStorage.getItem('userBookings');
            ...
            try {
                ...
                console.log('Reservation saved to localStorage:', booking);
            } catch (e) {
                console.error('Error saving to localStorage:', e);
                if (e.name === 'QuotaExceededError') {
                    alert('Speicherplatz voll. Bitte löschen Sie alte Daten oder kontaktieren Sie den Support.');
                } else {
                    alert('Fehler beim Speichern der Reservierung. Bitte versuchen Sie es erneut.');
                }
                return; // Don't continue if localStorage save failed
            }

            const token = sessionStorage.getItem('token') || localStorage.getItem('token');
            if (token && selectedVehicle && selectedVehicle.car_id) {
                try {
                    const backendBooking = {
                        ...
                        pickupDate: pickupDate ? pickupDate.split('T')[0] : pickupDate,
                        pickupTime: booking.pickupTime,
                        dropoffDate: dropoffDate ? dropoffDate.split('T')[0] : dropoffDate,
                    ...
                    fetch('/api/reservations/create', {
                        ...
                        if (response.ok) {
                            ...
                            console.warn('Failed to save reservation to backend:', response.status);
                        }
                    }).catch(error => {
            ...
            console.log('Reservation saved:', booking);
        } catch (error) {
