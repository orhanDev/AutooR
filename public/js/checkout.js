document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('total-price').textContent = `ä${Math.floor(total).toLocaleString('de-DE')}`;
  ...
  function openSummary() {
    ...
    summary.innerHTML = `
      ...
      <div class="row"><div class="col-6">Gesamt</div><div class="col-6 text-end" id="summary-total">ä${Math.floor(total).toLocaleString('de-DE')}</div></div>
    ...
    const recalc = () => {
      ...
      extrasTotalEl.textContent = `ä${Math.floor(ext).toLocaleString('de-DE')}`;
      const newTotal = ext + Number(total);
      totalEl.textContent = `ä${Math.floor(newTotal).toLocaleString('de-DE')}`;
