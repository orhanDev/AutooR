(function(global){
  ...
  function guessFromPath(path){
    try{
      ...
      const map = { vw:'VW', volkswagen:'Volkswagen', bmw:'BMW', audi:'Audi', mercedes:'Mercedes', 'mercedes-benz':'Mercedes', porsche:'Porsche', cupra:'Cupra', peugeot:'Peugeot', range:'Range Rover', rover:'Range Rover', bugatti:'Bugatti' };
      let make = '';
      let start = 1;
  ...
  function resolveVehicleImage(v){
    if (v?.make && v?.model) {
      const aliasKey = `${(v.make||'').toLowerCase()} ${(v.model||'').toLowerCase()}`.trim();
      const aliased = ALIAS[aliasKey];
      if (aliased) {
        return `/images/cars/${aliased}`;
      }
    }
