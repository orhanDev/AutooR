// Shared image resolution utilities
(function(global){
  const IMG_INDEX = [
  ...
  // Explicit aliases for tricky names → exact filenames in /images/cars
  const ALIAS = {
  ...
  function resolveVehicleImage(v){
    ...
    // If provided path is not in our index, fallback to alias/best match
    if (!inIndex(img)) {
