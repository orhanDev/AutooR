...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setFieldInvalid(input, message) {
        ...
        if (feedback) {
            feedback.textContent = message || 'Bitte prüfen Sie dieses Feld.';
    ...
    if (confirmPasswordInput) {
        confirmPasswordInput.addEventListener('input', () => {
            ...
            if (!confirmPwd) {
                ...
                const fb = confirmPasswordInput.closest('.form-group')?.querySelector('.invalid-feedback');
                if (fb) {
                    fb.textContent = '';
            ...
            if (pwd === confirmPwd) {
                ...
                setFieldInvalid(confirmPasswordInput, 'Passwörter stimmen nicht überein.');
            }
        });
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (confirmPasswordInput) {
            ...
            if (!confirmPwd) {
                setFieldInvalid(confirmPasswordInput, 'Bitte bestätigen Sie Ihr Passwort.');
                showAlert('Bitte bestätigen Sie Ihr Passwort.', 'danger');
                submitBtn.disabled = false;
                submitBtn.innerHTML = originalText;
                return;
            }
            if (passwordInput.value !== confirmPwd) {
                setFieldInvalid(confirmPasswordInput, 'Passwörter stimmen nicht überein.');
        ...
        try {
            ...
            if (response.ok && result.token) {
                ...
                if (result.user) {
                    ...
                    const shouldPreserveData = existingEmail && existingEmail.toLowerCase() === registerEmail.toLowerCase();

                    const hasLocalStorageData = Object.keys(existingUserData).length > 0 || Object.keys(existingCurrentUser).length > 0;

                    

                    console.log('Register email:', registerEmail, 'Existing email:', existingEmail, 'Preserve data:', shouldPreserveData);

                    console.log('Register - hasLocalStorageData:', hasLocalStorageData);

                    console.log('Register - existingUserData.phone:', existingUserData.phone);

                    console.log('Register - existingCurrentUser.phone:', existingCurrentUser.phone);

                    const userDataToStore = {

                        ...(shouldPreserveData ? existingUserData : {}),
                        ...
                        id: result.user.id || result.user.user_id || existingUserData.id,
                    ...
                    if (shouldPreserveData) {
                        ...
                        currentUserToStore = {
                            ...
                            ...existingCurrentUser, // This preserves ALL existing fields (phone, birthDate, gender, street, postalCode, city, country, etc.)

                            firstName: result.user.first_name || existingCurrentUser.firstName,

                            lastName: result.user.last_name || existingCurrentUser.lastName,

                            email: result.user.email

                        };

                    } else if (hasLocalStorageData && existingEmail && existingEmail.toLowerCase() === registerEmail.toLowerCase()) {
                    ...
                    console.log('Register - Saved currentUserToStore:', currentUserToStore);

                    console.log('Register - Saved phone:', currentUserToStore.phone);

                    console.log('Register - Saved birthDate:', currentUserToStore.birthDate);
